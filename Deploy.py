...
def main():
    ...
    while True:
        ...
        if len(lm_list):
            ...
            # index + middle + ring finger -- save drawing (black bg + drawing - easy for taking notes)
            ...
            # index + middle + pinky finger (hold 3 seconds) -- Clear screen
            elif fingers[0] == 1 and fingers[1] == 1 and fingers[2] == 0 and fingers[3] == 1:
                if clear_gesture_start is None:
                    clear_gesture_start = time.time()
                elif time.time() - clear_gesture_start >= 3:
                    canvas = np.zeros((height, width, 3), dtype='uint8')
                    xp, yp = 0, 0
                    clear_gesture_start = None

            # index + middle + ring + pinky finger -- eraser
